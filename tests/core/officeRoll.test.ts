...
describe("rollWorkerWeights", () => {
  ...
  it("goldsmith: sell + combo in [3, 7]; speed in [0, 2]; crit_chunks in [0, 1]", () => {
    setSeed(11);
    for (let i = 0; i < 100; i++) {
...
describe("rollWorkerAffixes", () => {
  it("rolls exactly tier-slot-count affixes for a legendary (5)", () => {
    setSeed(20);
    const weights = { "+sell_price%": 1, "+speed%": 1, "+crit_chunks": 1, "+combo_chance%": 1 };
  ...
  it("rolls 1 affix for common", () => {
    setSeed(21);
    const weights = { "+sell_price%": 2, "+speed%": 2, "+crit_chunks": 2, "+combo_chance%": 2 };
  ...
  it("respects per-worker weights (high-weight kinds dominate)", () => {
    setSeed(22);
    const weights = { "+sell_price%": 100, "+speed%": 0, "+crit_chunks": 0, "+combo_chance%": 0 };
    const affixes = rollWorkerAffixes(weights, "legendary", stub());
