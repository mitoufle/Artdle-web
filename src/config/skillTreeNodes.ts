...
export const SKILL_NODES: ReadonlyArray<SkillNodeConfig> = [
  ...
  { id: "hire_manager", clusterId: "office", name: "Hire Manager", description: "unlock an additional worker slot", numericEffect: "1", parentIds: ["robin_hood"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [4500], unlocks: ["roster_slot"] },
  // ── Office worker-progression nodes (2026-06-01 design submission). These
  //    boost a worker's SPAWN base stats / XP curve; wiring lives in
  //    officeSlice (getWorkerSpawnBonuses / getWorkerXpGrowth) + workerModel.
  { id: "food_regulation", clusterId: "office", name: "Food Regulation", description: "newly-hired workers gain +1 to every base stat (+1% crit chance).", numericEffect: "1", parentIds: ["entrepreneur"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [0], unlocks: ["worker_food_regulation"] },
  { id: "robin_hood", clusterId: "office", name: "Robin Hood", description: "newly-hired workers gain +7% Gold base stat per level.", numericEffect: "7%", parentIds: ["food_regulation"], stacking: "additive", kind: "minor", maxLevel: 5, costs: [5000, 7500, 10000, 15000, 20000], unlocks: ["worker_goldpct_base"] },
  { id: "blury_hand", clusterId: "office", name: "Blurry Hand", description: "newly-hired workers gain +10% stroke speed base stat.", numericEffect: "10%", parentIds: ["entrepreneur"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [7500], unlocks: ["worker_speed_base"] },
  { id: "learning_curve", clusterId: "office", name: "Learning Curve", description: "lowers the worker XP ramp-per-level by 0.5 (flatter leveling curve).", numericEffect: "0.5", parentIds: ["accelerator"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [6000], unlocks: ["worker_xp_growth_reduction"] },
  // employer_branding + sp500 grant roster slots (their description says so);
  // the new_node_* entries are inert authored placeholders (no effect yet),
  // mirrored so the JSON↔runtime agreement + cluster invariants hold.
  ...
  { id: "new_node_8", clusterId: "office", name: "New Node", description: "", numericEffect: "", parentIds: ["hire_manager"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [0], unlocks: [] },
  { id: "new_node_9", clusterId: "office", name: "New Node", description: "", numericEffect: "", parentIds: ["employer_branding"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [0], unlocks: [] },
  { id: "sp500", clusterId: "office", name: "SP500", description: "unlocks an additional worker slot", numericEffect: "1", parentIds: ["new_node_8", "new_node_9"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [0], unlocks: ["roster_slot"] },
  ...
  { id: "new_node_13", clusterId: "office", name: "New Node", description: "", numericEffect: "", parentIds: ["hire_manager"], stacking: "additive", kind: "minor", maxLevel: 1, costs: [0], unlocks: [] },
  ...
  { id: "enlightenment", clusterId: "inspiration", name: "Enlightenment", description: "Each level reduces the inspiration needed to ascend by 5%.", numericEffect: "-5%", parentIds: ["get_inspired"], stacking: "additive", kind: "minor", maxLevel: 4, costs: [8, 15, 25, 40], unlocks: ["ascend_threshold_reduction"] },
  { id: "patron", clusterId: "inspiration", name: "Patron", description: "Each level boosts inspiration gain by +10% (stacks with Get Inspired).", numericEffect: "10%", parentIds: ["get_inspired"], stacking: "additive", kind: "minor", maxLevel: 5, costs: [8, 15, 25, 40, 60], unlocks: ["inspi_mult_bonus"] },
