...
export const SKILL_NODES: ReadonlyArray<SkillNodeConfig> = [
  ...
  { id: "basic_technique", name: "Basic Technique", description: "Each level makes canvas painting 10% faster.", numericEffect: "10%", parentIds: [], stacking: "additive", kind: "minor", maxLevel: 5, costs: [1, 2, 3, 4, 5], unlocks: [] },
  { id: "muscle_memory", name: "Muscle Memory", description: "Each level makes canvas painting 10% faster.", numericEffect: "10%", parentIds: ["basic_technique"], stacking: "additive", kind: "minor", maxLevel: 5, costs: [3, 5, 8, 12, 20], unlocks: [] },
  ...
  { id: "entrepreneur", name: "Entrepreneur", description: "unlocks the Worker Office tab ", numericEffect: "1", parentIds: ["forget_pain"], stacking: "additive", kind: "major", maxLevel: 1, costs: [700], unlocks: ["roster_slot"] },
  ...
  { id: "hire_manager", name: "Hire Manager", description: "Each level adds +1 roster slot for hired workers.", numericEffect: "+1", parentIds: ["entrepreneur"], stacking: "additive", kind: "minor", maxLevel: 4, costs: [4500, 6000, 8000, 10000], unlocks: ["roster_slot"] },
  { id: "accelerator", name: "Accelerator Program", description: "Each level boosts the worker ascend-XP pool by +10%.", numericEffect: "10%", parentIds: ["entrepreneur"], stacking: "additive", kind: "minor", maxLevel: 5, costs: [4500, 6000, 7500, 9000, 10000], unlocks: ["worker_xp_mult"] },
  ...
  { id: "unlock_school", name: "Painting School", description: "Unlocks the Painting School — research permanent bonuses one at a time.", numericEffect: "", parentIds: ["accelerator"], stacking: "additive", kind: "major", maxLevel: 1, costs: [25000], unlocks: ["school_access"] },
