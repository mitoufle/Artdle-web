...
export const getWorkerGoldFactor = (state: Pick<GameStore, "roster" | "purchasedNodes">): number => {
  const bonuses = getWorkerBaseStatBonuses(state);
  let factor = 1;
  for (const w of state.roster) factor *= 1 + applyBaseStatBonuses(w.stats, bonuses).goldPct;
  return factor;
};
 ...
 */
export const getWorkerBaseStatBonuses = (state: Pick<GameStore, "purchasedNodes">): WorkerBaseStatBonuses => ({
