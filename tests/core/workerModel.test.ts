...
import { createBaseStats, applyBaseStatBonuses, applyStatLevelUp, type WorkerStats } from "@/core/workerModel";
...
describe("workerModel", () => {
  ...
  it("applyBaseStatBonuses with no bonuses leaves stats unchanged", () => {
    expect(applyBaseStatBonuses(createBaseStats())).toEqual(WORKER_BASE_STATS);
  });

  it("food_regulation adds +1 native step to every base stat", () => {
    const s = applyBaseStatBonuses(createBaseStats(), { foodRegulation: 1, robinHoodLevels: 0, bluryHandLevels: 0, handcraftedBrushLevels: 0 });
  ...
  it("robin_hood adds +7% goldPct per level; blury_hand adds +10% speed", () => {
    const s = applyBaseStatBonuses(createBaseStats(), { foodRegulation: 0, robinHoodLevels: 3, bluryHandLevels: 1, handcraftedBrushLevels: 0 });
  ...
  it("handcrafted_brush adds +3% speed per level, stacking with blury_hand", () => {
    const s = applyBaseStatBonuses(createBaseStats(), { foodRegulation: 0, robinHoodLevels: 0, bluryHandLevels: 1, handcraftedBrushLevels: 5 });
  ...
  it("applies bonuses ON TOP of a worker's own (leveled) stats, not just base", () => {
    ...
    const s = applyBaseStatBonuses(leveled, { foodRegulation: 0, robinHoodLevels: 2, bluryHandLevels: 0, handcraftedBrushLevels: 0 });
  ...
  it("stacks food_regulation + robin_hood on goldPct and clamps crit at the cap", () => {
    const s = applyBaseStatBonuses(createBaseStats(), { foodRegulation: 1, robinHoodLevels: 5, bluryHandLevels: 0, handcraftedBrushLevels: 0 });
